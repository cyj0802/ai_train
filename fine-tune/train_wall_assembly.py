...
BASE = Path(r"T:\03_Platform\03.Floorplans\Train\Wall\wall assembly")

# wall_data.yaml 이 저장될 위치
DATA_YAML_PATH = BASE / "wall_data.yaml"

# LabelMe json이 같은 구조로 BASE 아래에 있다고 가정
SPLITS = ["train", "val", "test"]

# 🔥 여기에는 "초기 학습에 사용할 가중치" 경로 넣어줘
# 1) COCO 사전학습된 YOLO11 세그 모델로 시작 (일반적인 케이스):
MODEL_WEIGHTS = "yolo11m-seg.pt"
# 2) 완전 랜덤 초기화로 가고 싶으면 (선택):
#    - yolo11m-seg.yaml 을 사용해서:
# MODEL_WEIGHTS = "yolo11m-seg.yaml"
...
def normalize_label(s: str) -> str:
    ...
    s1 = s0.replace(" ", "")
    s2 = s1.lower()
    # r1~r10 같은 건 무시 (여기선 wall, glass만 사용)
...
"""
    ...
    print("✅ wall_data.yaml 저장 완료 →", yaml_path, "\n")
...
def main():
    ...
    # 4) 모델 로드 (초기 학습: COCO-pretrained 또는 yaml 기반)
    #    MODEL_WEIGHTS에 "yolo11m-seg.pt" 를 넣으면 COCO 사전학습에서 시작 (추천)
    #    "yolo11m-seg.yaml" 을 넣으면 랜덤 초기화에서 시작
    model = YOLO(str(MODEL_WEIGHTS))

    run_name = "wall_seg_train_local"   # ft/fine-tune 문구 제거

    # 5) 학습
    results = model.train(
        data=str(DATA_YAML_PATH),
        epochs=300,                  # 단순 테스트라면 50~100 정도로 줄여도 됨
        imgsz=(1600, 1280),
        batch=4,
        device=device,
        project="runs_wall/segment_local",
        name=run_name,
        save=True,
        patience=100,
        optimizer="SGD",
